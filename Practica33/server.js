...
// Cambiamos un poco esta línea para exportar la app y decirle el puerto después
app.on('request', (request, response) => {
    ...
    try {
      for (let route of routes) {
        if (request.url.match(route[0]) && route[1][request.method.toLowerCase()]) {
          ...
          if (response.finished) return log();
        }
      }
      return sendError(401, -32601, "Method not found", request.body ? request.body.id : null);
    } catch (err) {
  ...
  function sendError(status, code, message, id = null) {
    ...
    response.end(JSON.stringify({ "jsonrpc": "2.0", "error": { code, message }, "id": id }));
    log();
  }
  function log() {
    console.log(new Date().toISOString(), request.connection.remoteAddress, response.statusCode, request.url, request.body && request.body.method);
  }
});

module.exports = function addRoute(path, methods) {
  routes.set(path, methods);
  return app; // Retornamos la app para poder hacerle .listen(puerto)
};